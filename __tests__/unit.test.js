...
describe("Error states and defaults", () => {
  it("should error with invalid record set", () => {
    ...
    expect(result).toThrow("Input must be array or object");
  });

  it("should error on missing `dynamodb` field", () => {
    const result = () => processor([{ test: true }]);
    expect(result).toThrow("Record is missing dynamodb property");
  });
});

describe("NEW_AND_OLD_IMAGES", () => {
  it("should unmarshall & check diff", () => {
    const event = require("./events/new-and-old.json");
    const result = processor(event.Records, true);
    ...
    expect(result[0].dynamodb.NewImage.BinarySet[0]).toEqual(
      Buffer.from("AAEqQQ==")
    );
    expect(result[0].dynamodb.NewImage.BinarySet[1]).toEqual(
      Buffer.from("AAEqQQ==")
    );
    ...
    expect(result[0].dynamodb.OldImage.BinarySet[0]).toEqual(
      Buffer.from("AAEqQQ==")
    );
    expect(result[0].dynamodb.OldImage.BinarySet[1]).toEqual(
      Buffer.from("AAEqQQ==")
    );
  ...
  it("should unmarshall & check additions", () => {
    const event = require("./events/new-and-old.json");
    const result = processor(event.Records, "added");
  ...
  it("should unmarshall & check updates", () => {
    const event = require("./events/new-and-old.json");
    const result = processor(event.Records, "updated");
  ...
  it("should unmarshall & check deletions", () => {
    const event = require("./events/new-and-old.json");
    const result = processor(event.Records, "deleted");
  ...
  it("should unmarshall & calc detailed diff", () => {
    const event = require("./events/new-and-old.json");
    const result = processor(event.Records, "detailed");
  ...
  it("should unmarshall & do not check diff", () => {
    const event = require("./events/new-and-old.json");
    const result = processor(event.Records);
  ...
  it("should unmarshall with wrapNumbers options", () => {
    const event = require("./events/new-and-old.json");
    const result = processor(event.Records, false, { wrapNumbers: true });

    expect(result[0].dynamodb.NewImage.FloatNumber).toBeInstanceOf(NumberValue);
    expect(result[0].dynamodb.NewImage.IntegerNumber).toBeInstanceOf(
      NumberValue
    );
    expect(result[0].dynamodb.NewImage.List[2]).toBeInstanceOf(NumberValue);
    expect(result[0].dynamodb.NewImage.NumberSet[0]).toBeInstanceOf(
      NumberValue
    );
  });
  ...
  it("should unmarshall with wrapSets options", () => {
    const event = require("./events/new-and-old.json");
    const result = processor(event.Records, false, { wrapSets: true });
...
describe("NEW_IMAGE", () => {
  it("unmarshall & check diff (no diff)", () => {
    const event = require("./events/new.json");
    const result = processor(event.Records, true);
  ...
  it("unmarshall & do not check diff", () => {
    const event = require("./events/new.json");
    const result = processor(event.Records, false);
...
describe("OLD_IMAGE", () => {
  it("unmarshall & check diff (empty object)", () => {
    const event = require("./events/old.json");
    const result = processor(event.Records, true);
...
describe("KEYS_ONLY", () => {
  it("unmarshall & check diff (no diff exists)", () => {
    const event = require("./events/keys-only.json");
    const result = processor(event.Records, true);
...
describe("Multiple Records", () => {
  it("unmarshalls & checks diffs for multiple records", () => {
    const event = require("./events/multiple-records.json");
    const result = processor(event.Records, true);
